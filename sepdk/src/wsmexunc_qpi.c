...
)
{
    ...
    FOR_EACH_REG_ENTRY_UNC(pecb_unc, dev_idx, i) {
         ...
         * Disable counting
         */
        if (ECB_entries_reg_id(pecb_unc,i) == QPI_PERF_GLOBAL_CTRL) {
            SYS_Write_MSR(ECB_entries_reg_id(pecb_unc,i), (long long)QPI_GLOBAL_DISABLE);
            continue;
        }

        SYS_Write_MSR(ECB_entries_reg_id(pecb_unc,i), ECB_entries_reg_value(pecb_unc,i));
#if defined(MYDEBUG)
        SEP_PRINT_DEBUG("wsmexunc_qpi_Write_PMU Event_Data_reg = 0x%x --- value 0x%llx\n",
...
{
    SYS_Write_MSR(QPI_PERF_GLOBAL_CTRL, (long long)QPI_GLOBAL_DISABLE);
...
)
{
    ...
    if (GLOBAL_STATE_current_phase(driver_state) == DRV_STATE_RUNNING) {
        SYS_Write_MSR(QPI_PERF_GLOBAL_CTRL, (long long)QPI_GLOBAL_ENABLE);
    }
...
/*!
 * @fn         static VOID wsmexunc_qpi_Clean_Up(PVOID)
 *
...
/* ------------------------------------------------------------------------- */
/*!
 * @fn wsmexunc_qpi_Read_Counts(param, id)
 *
