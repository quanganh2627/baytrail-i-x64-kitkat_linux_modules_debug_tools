...
)
{
    ...
    SEP_PRINT_DEBUG("Entered montecito_Write_PMU.\n");
...
)
{
    ...
    SEP_PRINT_DEBUG("Entered montecito_Disable_PMU.\n");
 ...
 *
 *             Event collection for a generic monitor is enabled under
 ...
 *             2) Clear the overflow bits 4-15.
 *         
...
)
{
    ...
    SEP_PRINT_DEBUG("Entered montecito_Enable_PMU.\n");
    ...
    if (GLOBAL_STATE_current_phase(driver_state) == DRV_STATE_RUNNING) {
        ...
        /* Clear the overflow bits 4-15 and the freeze bit (bit 0). */
        /* Per vdk1 sources:
           Mask pmi before setting pmc0.fr=1 to workaround Itanium(R) 2 processor errata.
           Setting pmc0.fr=1 on Itanium(R) 2 processor can cause a pmi.  While pmi is masked via
           the pmv, pmi requests will be ignored (not latched). */
// ???
        ...
        val = SYS_Read_PMC(IA64_PMC0);
        SYS_Write_PMC(IA64_PMC0, val & ~((U64)0xfff1));
#if defined(MYDEBUG)
        {
            U64 val2     = SYS_Read_PMC(IA64_PMC0);
            U32 this_cpu = CONTROL_THIS_CPU();
            SEP_PRINT_DEBUG("On CPU%d: Write PMC 0x%x --- value 0x%llx -- read 0x%llx\n",
                            this_cpu, IA64_PMC0, val & ~((U64)0xfff1), val2);
        }
#endif
        SYS_Clear_PMV_Mask();
    }
...
)
{
    ...
    SEP_PRINT_DEBUG("Entered montecito_Read_PMU_Data.\n");
...
)
{
    ...
    FOR_EACH_DATA_GENERIC_REG(pecb, i) {
        ...
        if (IA64_PMC4 <= ECB_entries_reg_id(pecb, i) && ECB_entries_reg_id(pecb, i) <= IA64_PMC15) {
    ...
    /* Clear overflow bits 4-15. */
    SYS_Write_PMC(IA64_PMC0, overflow_status & ~((U64)0xfff0));
    SEP_PRINT_DEBUG("Check Overflow completed %d\n", this_cpu);
}


static VOID
montecito_Initialize (
    VOID  *param
)
{
    SEP_PRINT_DEBUG("Entered montecito_Initialize.\n");
}
...
{
    SEP_PRINT_DEBUG("Entered montecito_Destroy.\n");
}
...
)
{
    ...
    SEP_PRINT_DEBUG("Entered montecito_Read_RO.\n");
 ...
 */
DISPATCH_NODE  montecito_dispatch =
{
    montecito_Initialize,        // init
    montecito_Destroy,           // fini
    montecito_Write_PMU,         // write
    montecito_Disable_PMU,       // freeze
    montecito_Enable_PMU,        // restart
    montecito_ReInit_Data,       // reinit
    montecito_Read_PMU_Data,     // read_data
    montecito_Check_Overflow,    // check_overflow
    NULL,                        // swap_group
    NULL,                        // read_lbrs
    montecito_Clean_Up,          // clean_up
    NULL,                        // hw_errata
    NULL,                        // read_power
    NULL,                        // check_overflow_errata
    NULL,                        // read_counts
    NULL,                        // check_overflow_gp_errata
    montecito_Read_RO,           // read_ro - used for dear, iear, btb, ipear
    NULL                         // platform_info
};
